...
def main():
    ...
    print(f"churn scenario: {args.n_base} settled base vehicles + 1 late-detected, "
          f"deadline-competitive disruptor, n_scenarios={args.n_scenarios}\n")

    methods = {"edf": edf_select, "hgt": hgt_select}
    results = {name: {"wt": [], "disruptor_wt": [], "base_wt": []} for name in methods}
    ...
    for arrivals in scenarios:
        ...
        for name, selector in methods.items():
            ...
            if base_vals:
                results[name]["base_wt"].append(sum(base_vals) / len(base_vals))

    def mean(xs):
        return sum(xs) / len(xs) if xs else float("nan")

    print(f"{'method':8s} {'overall_wt':>12s} {'disruptor_wt':>14s} {'base_avg_wt':>14s}")
    for name in methods:
        r = results[name]
        print(f"{name:8s} {mean(r['wt']):12.4f} {mean(r['disruptor_wt']):14.4f} {mean(r['base_wt']):14.4f}")
